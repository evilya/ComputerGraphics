...
public class Matrix implements Cloneable {
    ...
    public Matrix subMatrix(int x0, int y0, int width, int height) {
        if (x0 < 0 || y0 < 0
                || width <= 0 || height <= 0
                || x0 + width > this.width || y0 + height > this.height) {
    ...
    public double scalarProduct(Matrix val) {
        if (this.height != val.height || this.width != val.width) {
            throw new IllegalArgumentException();
        }
    ...
    public Matrix normalize() {
        double sum = Math.sqrt(Arrays.stream(data).map(e -> e * e).sum());
